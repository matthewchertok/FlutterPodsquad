...
Widget viewPersonDetailsDrawer(
        ...
        required ProfileData personData, required List<PodData> personsPodMemberships}) =>
    Drawer(
        child: SafeArea(child: ListView(
          ...
          children: [
            ...
            ListTile(
              title: Text('Interact with ${personData.name}!'), subtitle: Text("${personData.name.firstName()}'s "
                "podscore: ${personData.podScore}"),
            ),

            // view their extra images, if they have any
            if (personData.extraImagesList?.isNotEmpty ?? false)
              ListTile(title: Text("${personData.name.firstName()}'s Photos"), leading: Icon(CupertinoIcons.photo), onTap:
                  (){
            ...
            if (personData.userID != myFirebaseUserId)
              ListTile(title: Text("Add ${personData.name.firstName()} to a pod"), leading: Icon(CupertinoIcons
                  .plus), onTap: (){
                Navigator.of(context, rootNavigator: true).push(CupertinoPageRoute(
                    builder: (context) => MainListDisplayView(
                        viewMode: MainListDisplayViewModes.addPersonToPod, personData: personData)));
              },),
